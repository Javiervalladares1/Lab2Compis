...
- Si el archivo tiene **errores sintácticos**, ANTLR los imprime en consola (línea, columna y descripción) y el chequeo de tipos no se ejecuta.
...
Un programa es **una o más instrucciones** (`stat`). Es la regla desde la que ambos drivers inician el análisis (`parser.prog()`).
...
# >> El programa tiene errores sintacticos: no se ejecuta el chequeo de tipos.
```

## Resultado esperado

- Si el programa es **sintácticamente correcto y sin errores de tipo**, aparece el mensaje de validación exitosa (`>> La validacion de tipos fue exitosa ...`).
- Si el programa tiene **errores sintácticos**, ANTLR los muestra en consola con línea y columna, y el chequeo de tipos no se ejecuta.
